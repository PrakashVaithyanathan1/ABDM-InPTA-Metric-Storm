...
psd = np.abs(np.fft.fft(dm))**2
freqs = np.fft.fftfreq(len(dm))
idx = np.argsort(freqs)
f, p = freqs[idx][freqs[idx]>0], psd[idx][freqs[idx]>0]

# 3. Final Alpha Slope Calculation
log_f, log_p = np.log10(f), np.log10(p)
slope, _ = np.polyfit(log_f, log_p, 1)

print(f"DM TIME SERIES ALPHA SIGNATURE: {abs(slope):.4f}")
...
print(f"DM ALPHA SIGNATURE: {alpha_val:.4f}")
